...
def risk_manager():
    rm = RiskManager()
    asyncio.get_event_loop().run_until_complete(rm.initialize(50.0))
    return rm
...
def test_kill_switch(risk_manager):
    asyncio.get_event_loop().run_until_complete(
        risk_manager._trigger_kill_switch("test")
    )
